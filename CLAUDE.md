...
- Это форк от `ShuNegus/sing-box-for-apple`, ветка `dev`. TURN-интеграция (вариант B) **уже влита** и работает на девайсе; дальше — доработки (напр. webview-капча).
- **Ядро sing-box тоже чистовое** — лежит рядом с этим репо: `../sing-box/` (т.е. `clients/ios/sing-box/`). Из него собирается `Libbox.xcframework`.
  - **Состояние ядра:** клон `SagerNet/sing-box` `v1.14.0-alpha.30`, ветка `bublik-dev`, **с влитым `vk-turn` outbound** (нативный, вариант B; `protocol/vkturn` + go.mod replace на `Turn/References/.../vk-turn-proxy`). Отдельный git-репо, от `x-ray/` не зависит.
  - `Libbox.xcframework` собран из этого ядра (с `vk-turn`) и лежит в клиенте (gitignored). **ВАЖНО:** при изменениях в ядре Libbox надо пересобирать (раздел «Сборка») — иначе клиент линкуется со старым ядром (симптом: `unknown outbound type: vk-turn`).
