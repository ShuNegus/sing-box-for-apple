...
- **Ядро sing-box тоже чистовое** — лежит рядом с этим репо: `../sing-box/` (т.е. `clients/ios/sing-box/`). Из него собирается `Libbox.xcframework` (туда вливается `vk-turn-proxy` clientcore).
...
- Детали интеграции — корневой CLAUDE.md, раздел «iOS/macOS-клиент … + TURN» и `../../../Turn/TURN_SINGBOX_PLAN.md` (источник истины).
...
- Подробный журнал TURN-задачи — `../../../Turn/TURN_SINGBOX_PLAN.md` (держать в актуальном состоянии).
